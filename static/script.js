...
function initDragAndDrop() { const ta = document.getElementById('node-text'); if (!ta) return; ta.addEventListener('dragover', e => { e.preventDefault(); ta.style.border = '1px dashed var(--accent)'; }); ta.addEventListener('dragleave', e => { e.preventDefault(); ta.style.border = '1px solid var(--border-color)'; }); ta.addEventListener('drop', async e => { e.preventDefault(); ta.style.border = '1px solid var(--border-color)'; if(e.dataTransfer.files && e.dataTransfer.files.length > 0) { const f = e.dataTransfer.files[0]; uploadWithProgress(f, (data) => { if(data.filename) { let txt = f.type.startsWith('image/') ? `[img:${data.filename}]` : `[file:${data.filename}|${data.original}]`; const s = ta.selectionStart; ta.value = ta.value.substring(0, s) + txt + ta.value.substring(ta.selectionEnd); ta.focus(); ta.setSelectionRange(s + txt.length, s + txt.length); } }); } }); ta.addEventListener('paste', async e => { const items = e.clipboardData && e.clipboardData.items; if (!items) return; for (const item of items) { if (item.type.startsWith('image/')) { e.preventDefault(); const file = item.getAsFile(); if (!file) return; uploadWithProgress(file, (data) => { if (data.filename) { const txt = `[img:${data.filename}]`; const s = ta.selectionStart; ta.value = ta.value.substring(0, s) + txt + ta.value.substring(ta.selectionEnd); ta.focus(); ta.setSelectionRange(s + txt.length, s + txt.length); } }); return; } } }); }
